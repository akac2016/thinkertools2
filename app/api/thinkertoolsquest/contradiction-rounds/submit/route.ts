  ...
  try {
    ...
    if (!activityData) {
      return jsonError("Contradiction round not found", {
        status: 404,
        code: "QUESTS_ACTIVITY_NOT_FOUND",
      });
    }
    ...
    if (visibleLabels.length < 2 || correctAnswerLabels.length !== 2) {
      return jsonError("Contradiction round is misconfigured", {
        status: 500,
        code: "QUESTS_ACTIVITY_CONFIG_INVALID",
      });
    }
    ...
    if (clickSelectionIsValid) {
      ...
      if (typedMatch.status === "invalid_input") {
        return jsonError(typedMatch.recoverableMessage, {
          status: 422,
          code: "QUESTS_INVALID_TYPED_SELECTION",
          details: {
      ...
      return jsonError(RECOVERABLE_INVALID_INPUT_MESSAGE, {
        status: 422,
        code: "QUESTS_SELECTION_REQUIRED",
        details: {
    ...
    if (awardedXp > 0) {
      ...
      if (updateProgressResult.error) {
        return jsonError("Failed to update quest progress", {
          status: 500,
          code: "QUESTS_PROGRESS_UPDATE_FAILED",
          details: {
    ...
    if (completionResult.error) {
      return jsonError("Failed to record contradiction round completion", {
        status: 500,
        code: "QUESTS_COMPLETION_RECORD_FAILED",
        details: {
